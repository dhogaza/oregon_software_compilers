...
function _p_cstring(src: _p_shortstring): _p_charptr;

{ Calls to this should be generated by the compiler, because
  when called directly from pascal code it will copy the src
  string before the call because src is marked "modified", due
  to the ref() on it.  This also prevents it being declared as
  a const parameter.
}
